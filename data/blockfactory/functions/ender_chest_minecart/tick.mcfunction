execute as @e[type=minecart] at @s if entity @e[type=item,nbt={Item:{id:"minecraft:ender_chest",Count:1b}},distance=..2] anchored eyes rotated as @s positioned as @s run function blockfactory:ender_chest_minecart/summon
